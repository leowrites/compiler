...
struct Alloca2RegPass : public FunctionPass {
    ...
    std::map<BasicBlock *, std::map<AllocaInst *, Value *>>
        Post;  // value at the end of the BB, could be store value or a phi node
    std::map<BasicBlock *, std::map<AllocaInst *, PHINode *>>
        Pre;  // value before the BB
    ...
    template <typename T>
    void accumulateUnusedInstructions(Function &F,
                                      std::vector<Instruction *> &toRemove) {
        // errs() << "accumulating unused instructions for " << F.getName() <<
        // "\n";
        for (BasicBlock &BB : F) {
            for (Instruction &inst : BB) {
                // for store, there may not be users, but we don't want to
                // remove it unless it's using alloca
                if (isa<StoreInst>(inst)) {
                    if (TargetAllocas.find(dyn_cast<AllocaInst>(
                            getPointerOperand(&inst))) != TargetAllocas.end()) {
        ...
        // errs() << "finished accumulating unused instructions for " <<
        // F.getName() << "\n";
    }
        ...
        for (BasicBlock &BB : F) {
            ...
            for (AllocaInst *allocaInst : TargetAllocas) {
                ...
                if (!Pre[&BB][allocaInst]) {
                    ...
                    // this could be wrong, wha tis the correct type? Right now
                    // every phi node has i32
                    PHINode *phiNode =
                        PHINode::Create(allocaInst->getAllocatedType(),
                                        pred_size(&BB), name, &BB.front());
            ...
            for (auto innerEntry : entry.second) {
                ...
                for (auto predBB : predBBs) {
                    if (Post[predBB][allocaInst]) {
                        ...
                        // errs() << "PhiNode " << phiNode->getName() << "has no
                        // post value\n"; phiNode->getType()->print(errs());
                        // Post[predBB][allocaInst]->print(errs());
                        phiNode->addIncoming(
                            UndefValue::get(allocaInst->getAllocatedType()),
                            predBB);
                    }
                }
            }
    ...
    // Since the language doesn't support pointers, i don't think the value of
    // the phi node can be modified
    void removeSelfReference(Function &F) {
            ...
            for (PHINode *phi : phiNodes) {
                ...
                for (unsigned i = 0, e = phi->getNumIncomingValues(); i != e;
                    ...
                    if (!foundNonSelf) {
                        ...
                        // Found another non-self reference, check if it's
                        // different
                ...
                if (foundNonSelf && !multipleDistinctNonSelf &&
                    hasSelfReference) {
                    if (nonSelfValue) {
        ...
        // since a phi node could have other phi nodes as reference, which could
        // be deleted if a phi node references another phi node, follow the
        // chain until the node no longer references a phi node. Then, replace
        // the the nodes with the real value before we perform replacement
        for (auto &pair : nodesToRemove) {
            PHINode *phiToReplace = pair.first;
            Value *currVal = pair.second;

            // if the current value is a phi node, need to keep looking for a
            // value
    ...
    virtual bool runOnFunction(Function &F) {
        ...
        // can also check if phi nodes were stored to? if they weren't then they
        // would be constants and we can get rid of it
        return true;
    }
};
}  // namespace
...
static RegisterPass<Alloca2RegPass> X("alloca2reg",
                                      "Alloca-to-register pass for minic",
                                      false, false);
...
static RegisterStandardPasses RegisterMyPass(
    PassManagerBuilder::EP_EarlyAsPossible, registerAlloca2RegPass);